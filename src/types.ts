...
export interface Square {
  ...
  ownerId?: number | null; // Player ID of the owner (number to match user's code)
...
export interface Player {
  id: number; // Changed to number to match user's code
  name: string;
  ...
  bankruptedBy?: number; // ID of player who bankrupted this player
...
export interface GameState {
  ...
  votesToEnd: number[]; // Array of player IDs who voted to end (number to match user's code)
  currentPlayerId?: number; // ID of the current player (for multiplayer)
}